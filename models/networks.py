...
class Vgg19(nn.Module):
    def __init__(self, requires_grad=False):
        ...
        for x in vgg_pretrained_features.modules():#fix for torch >>v1.2
...
class TCM(nn.Module):
    def __init__(self, layids = None):
        ...
        self.flownet = flownet.FlowNet()
        self.flownet.cuda()
        self.criterion = nn.L1Loss()
 

    def forward(self, V1, V2):
        flow=self.flownet(V1,V2)
        ######
        return flow
...
def save_general_checkpoint(model,optimizer,save_path):
    if not os.path.exists(os.path.dirname(save_path)):
        os.makedirs(os.path.dirname(save_path))
    checkpoint={
...
def load_checkpoint_parallel(model, checkpoint_path,train_flag=False,optimizer=False):
    ...
    if train_flag:
        ...
        for param in checkpoint_new:
            checkpoint_new[param] = checkpoint_model[param]
...
def load_checkpoint_part_parallel(model, checkpoint_path,train_flag=False,optimizer=None):
